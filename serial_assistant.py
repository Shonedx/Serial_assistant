...
class ConsolePage(ctk.CTkFrame):
    def __init__(self, parent, controller):
        ...
        # 第一行：基础连接
        r1 = ctk.CTkFrame(cfg_bar, fg_color="transparent")
        ...
        # 第二行：高级参数
        r2 = ctk.CTkFrame(cfg_bar, fg_color="transparent")
        r2.pack(fill="x", pady=(0, 10), padx=10)

        ctk.CTkLabel(r2, text="数据位:").pack(side="left", padx=5)
        self.data_sel = ctk.CTkOptionMenu(r2, values=["8", "7", "6", "5"], width=80)
        self.data_sel.set("8"); self.data_sel.pack(side="left", padx=2)

        ctk.CTkLabel(r2, text="校验位:").pack(side="left", padx=10)
        self.parity_sel = ctk.CTkOptionMenu(r2, values=["None", "Even", "Odd", "Mark", "Space"], width=90)
        self.parity_sel.set("None"); self.parity_sel.pack(side="left", padx=2)

        ctk.CTkLabel(r2, text="停止位:").pack(side="left", padx=10)
        self.stop_sel = ctk.CTkOptionMenu(r2, values=["1", "1.5", "2"], width=80)
        self.stop_sel.set("1"); self.stop_sel.pack(side="left", padx=2)
        ...
        self.paned.add(self.recv_frame, height=500) # 初始高度
        ...
        if not c.running:
            try:
                ...
                c.ser = serial.Serial(
                    ...
                    baudrate=int(self.baud_sel.get()),
                    bytesize=int(self.data_sel.get()),
                    parity=parity_map[self.parity_sel.get()],
                    stopbits=stop_map[self.stop_sel.get()],
                    timeout=0.1
                )
                c.running = True
                ...
                self.recv_box.insert("end", f"⚠️ 连接失败: {e}\n")
...
class ParamComponent(ctk.CTkFrame):
    def __init__(self, parent, controller):
        ...
        self.name_e = ctk.CTkEntry(l1, placeholder_text="参数名", width=100); self.name_e.pack(side="left", padx=2)
...
class TextPacketComponent(ctk.CTkFrame):
    def __init__(self, parent, controller):
        ...
        self.header_row = ctk.CTkFrame(self, fg_color="transparent"); self.header_row.pack(fill="x", padx=10, pady=5)
        ...
        self.content_area = ctk.CTkFrame(self, fg_color="transparent"); self.content_area.pack(fill="both", expand=True, padx=10, pady=(0, 10))
    ...
    def toggle_fold(self):
        if not self.is_collapsed: self.content_area.pack_forget(); self.fold_btn.configure(text="🔽 展开"); self.is_collapsed = True
        else: self.content_area.pack(fill="both", expand=True, padx=10, pady=(0, 10)); self.fold_btn.configure(text="🔼 收起"); self.is_collapsed = False
    def toggle_mode(self, m):
        self.is_auto = (m == "自动")
        if self.is_auto: threading.Thread(target=self.worker, daemon=True).start()
    def worker(self):
